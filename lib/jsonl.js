...
function formatMessage(entry) {
  ...
  if (entry.type === 'user') {
    ...
    if (Array.isArray(content)) {
      ...
      // Check for local command pattern
      const localCmdMatch = content.match(/<command-name>([^<]+)<\/command-name>/);
      const outputMatch = content.match(/<local-command-stdout>([\s\S]*?)<\/local-command-stdout>/);

      if (localCmdMatch && outputMatch) {
        // This is a local command with output
        result.type = 'local_command';
        result.command = localCmdMatch[1];
