 ...
 */
async function readSession(sessionId, cwd) {
  const filePath = getSessionPath(sessionId, cwd);
 ...
 */
function watchSession(sessionId, callback, cwd) {
  const filePath = getSessionPath(sessionId, cwd);
  let position = 0;
  ...
  const processChanges = async () => {
    try {
      ...
      debounceTimer = setTimeout(async () => {
        const messages = await readSession(sessionId, cwd);
