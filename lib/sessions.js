...
async function sendInput(id, text, action) {
  ...
  // Answer multi-select (arrow-navigate to each option, toggle with Space, then Tab to Submit)
  // Claude Code's multiselect UI uses: "Enter to select · Tab/Arrow keys to navigate"
  // Number keys don't work - must use arrow keys to navigate between options
  if (action === 'answer-multi') {
    // text is comma-separated indices like "1,3" (1-based)
    const indices = text.split(',').filter(s => s.trim()).map(s => parseInt(s, 10)).sort((a, b) => a - b);

    // Cursor starts at option 1
    let currentPos = 1;

    for (const targetIdx of indices) {
      // Navigate down to target option
      const stepsDown = targetIdx - currentPos;
      for (let i = 0; i < stepsDown; i++) {
        await tmuxSpawn(['send-keys', '-t', name, 'Down']);
      }
      // Toggle selection with Space
      await tmuxSpawn(['send-keys', '-t', name, 'Space']);
